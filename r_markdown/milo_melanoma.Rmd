---
title: "Differential abundance analysis of melanoma data using milo"
...
We load the scRNA-seq data of immune cells from melanoma lesions treated with immune checkpoint inhibitors [Sade-Feldman et al. 2018](https://doi.org/10.1016/j.cell.2018.10.038) from the previously created `Seurat` object (see [here](preprocess_data_melanoma.md)). For use with milo, we transform the `Seurat` object into a `SingleCellExperiment` object.
...
We construct a KNN graph from the expression data and define cell neighborhoods according to the instructions of the authors of milo. We chose the parameters to achieve a neighborhood size distribution peaking between 50 and 100.
...
Then, we use milo to perform differential abundance testing on the defined neighborhoods.
...
We visualize differentially abundant neighborhoods at FDR = 0.1 on a UMAP projection and as a beeswarm plot.
...
For detailed comparison with the DICSIT results, we transform the neighborhood-level output into single-cell scores by averaging the logFC score of all neighborhoods that a cell belongs to.
