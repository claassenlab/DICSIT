---
title: "Differential expression analysis for selected cells in the COVID status classification"
...
For this analysis, we load the Seurat object containing the COVID-19 data from [Wilk et al. 2021](https://doi.org/10.1084/jem.20210582) and the extracted filter response from the disease status classification (i.e., Healthy vs. COVID).
...
For the first two filters, we add selected cells, i.e., cells with a filter response that is larger than $0.3 \times (\text{maximum filter response})$
