...
def load_data(path: str) -> pd.DataFrame:
    ...
    """
    data = pd.read_csv(path, index_col=0, sep=None)
...
def main():
    ...
    if args.feature_names is not None:
        feature_names = np.loadtxt(args.feature_names, dtype=str).tolist()
    else:
    ...
    np.random.seed(42)
    train_data = train_data.groupby(sample_col).apply(pd.DataFrame.sample, frac=1).reset_index(drop=True)
    test_data = test_data.groupby(sample_col).apply(pd.DataFrame.sample, frac=1).reset_index(drop=True)
